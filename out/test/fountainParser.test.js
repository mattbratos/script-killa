...
suite('Fountain Parser', () => {
    ...
    test('parses multi-line dialogue', () => {
        ...
        const blocks = (0, fountainParser_1.parseFountainDialogue)(text);
        assert.strictEqual(blocks.length, 1);
        assert.strictEqual(blocks[0].dialogueText, 'First line of dialogue.\nSecond line of dialogue.\nThird line.');
    });
    ...
    test('source range offsets are correct for multi-line dialogue', () => {
        ...
        const blocks = (0, fountainParser_1.parseFountainDialogue)(text);
        assert.strictEqual(blocks.length, 1);
        const extracted = text.substring(blocks[0].sourceRange.startOffset, blocks[0].sourceRange.endOffset);
        assert.strictEqual(extracted, 'Line one.\nLine two.');
    });
    test('getBaseCharacterName strips extensions', () => {
        assert.strictEqual((0, fountainParser_1.getBaseCharacterName)('MATT'), 'MATT');
        assert.strictEqual((0, fountainParser_1.getBaseCharacterName)('MATT (V.O.)'), 'MATT');
        assert.strictEqual((0, fountainParser_1.getBaseCharacterName)('SARAH (O.S.)'), 'SARAH');
        assert.strictEqual((0, fountainParser_1.getBaseCharacterName)("MATT (CONT'D)"), 'MATT');
    });
